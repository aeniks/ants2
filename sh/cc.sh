...
# CONTANTS
#===============================================================================
cyan=$(tput setaf 6); re=$(tput sgr0); dim=$(tput dim); bold=$(tput sgr0); rev=$(tput rev);
SELECTED=""$re"["$cyan"x"$re"]"
UNSELECTED="[ ]"
WHITE="\033[2K\033[37m"
WHITE="$re\033[2K\033[37m"
BLUE="\033[2K\033[34m"
RED="\033[2K\033[31m"
GREEN="\033[2K\033[32m"
INTERFACE_SIZE="2"
DEFAULT_OPTIONS=($(ls $yy -f1A 2>/dev/null))
#===============================================================================
# VARIABLES
#===============================================================================
cursor=2
...
selected_options=()
content="ssss"
message="\n  ["$dim"SPACE"$re"] to select - for help press ["$dim"h"$re"]"
separator="aa"
options_input="$(ls -fA1)"
color=
checkbox_output=()
read -ep "hello " -i "$PWD" "yy";
options_input=()
#===============================================================================
# UTILS
#===============================================================================
...
[ESC]:	     Exit
[UP]:	     Move cursor to option above
[DOWN]:	     Move cursor to option below
[HOME]:	     Move cursor to first option
[END]:	     Move cursor to last option
[PAGE UP]:   Move cursor 5 options above
[PAGE DOWN]: Move cursor 5 options below
[c]:         Copy current option
...
[INSERT]:    On/Off select options during navigation (select mode)
[BACKSPACE]: On/Off unselect options during navigation (unselect mode)"
fi
output+="\n\n(press q to quit)"
reset_screen
printf "\033[2J\033[?25l%b\n" "$output"
...
content+="$re $color $SELECTED $option \n$re"
else
content+="$re $color $UNSELECTED $option \n$re"
fi
}
