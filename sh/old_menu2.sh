...
ov1=($(ls $PWD/$1)); OPTIONS_VALUES=(${ov1[@]^^})
#OPTIONS_VALUES=(${ov2[@]//.*/ })
##################################################
##
...
checkbox SELECTED "$OPTIONS_STRING"
for i in "${!SELECTED[@]}"; do if [ "${SELECTED[$i]}" == "true" ]; then CHECKED+=("${OPTIONS_VALUES[$i]}"); fi; done; 
echo -e "\t You chose: \t ${CHECKED[@]/#/\\\n\\t" "}";
echo -ne "\n\t$c2 Do you wish to proceed? "$dim"["$re$bold"Y"$dim"/"$re$bold"n"$re$dim"] $re "; 
read -n1 -ep "" "yn"; if [ "$yn" != "${yn#[Nn]}" ]; then echo -e "$c2 nope"; 
return 0 2>/dev/null; break 2>/dev/null; return;
else echo -e "\n\t$c2 OK"; fi; for i in "${CHECKED[@],,}"; 
do echo -e "\t$c2 Installing $i"; sleep 1; done; echo -e "done";
